...
function Mapper(T)
{
  ...
  // bind as well
  this.toMany   = this.toMany.bind(this);
...
/**
 * Add a transform for the output
 * @param {function(input:object): object} f
 */
Mapper.prototype.useInput = function(f)
{
  return this.use(function(input, outut) {
    if (!input) return;
    f(input);
  });
};
...
/**
 * Add a transform for the input
 * @param {function(input:object): object} f
 */
Mapper.prototype.useOutput = function(f)
{
  return this.use(function(input, output) {
    if (!output) return;
    f(output);
  });
};
